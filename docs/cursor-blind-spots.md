...
| Policy gap on Read | Existing `credential-path-in-shell-args` matches **shell** args, not generic Read |
| Shell vs Read | Agent can `cat` sensitive path via Shell (gated) or Read tool (different hook/policy) |
